...
// query parameters
 ...
 // If get_user not available send error response to client in ivalid token
		...
		//get_senderid_allowed get whatspp_config_status not in ('D', 'B') is get the response to executed is continued.
			logger_all.info("[select query request] : " + `SELECT count(user_id) cntusr FROM rcs_config where whatspp_config_status not in ('D', 'B') and user_id = '${user_id}'`);
			const get_senderid_allowed = await db.query(`SELECT count(user_id) cntusr FROM rcs_config where whatspp_config_status not in ('D', 'B') and user_id = '${user_id}'`);
