...
async function WaitingApprovalList(req) {
	...
	try {
		...
		var waiting_senderid = `SELECT config.* FROM rcs_config config left join user_management usr on config.user_id = usr.user_id where usr.parent_id = '${user_id}' and config.whatspp_config_status = 'N'`;
		...
		let waiting_template;
		if (user_master_id == 1 && user_id == 1) { // primary admin are following this to use in the condition
			waiting_template = `SELECT * FROM message_template where template_status = 'N'`;
		} else if (user_master_id == 1) {
			// to get_approve_rcs_no_api using
			waiting_template = `SELECT temp.* FROM message_template temp left join user_management usr on temp.created_user = usr.user_id where usr.parent_id = '${user_id}' and temp.template_status = 'N'`;
		}
		...
		let userIdQuery;
		if (user_master_id == 1 && user_id == 1) { // primary admin are following this to use in the condition
			userIdQuery = `SELECT user_id FROM rcs.user_management`;
		} else if (user_master_id == 1) {
			// to get_approve_rcs_no_api using
			userIdQuery = `SELECT user_id FROM rcs.user_management where parent_id = '${user_id}'`;
		}
		...
		waiting_compose = waiting_compose + `SELECT user_id, compose_rcs_id FROM rcs.master_compose_rcs WHERE rcs_status = 'W' and user_id in ('${array_list_userid_string}') `;
