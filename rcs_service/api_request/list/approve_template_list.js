...
async function approveTemplateList(req) {
    ...
    try {
        ...
        var template_list = `SELECT * from message_template mt left join user_management usr on mt.created_user = usr.user_id left join master_language ml on ml.language_id = mt.language_id where mt.template_status = 'N' order by template_entdate desc`;
