...
async function getSingleTemplate(req) {
  try {
    ...
    logger_all.info("[select query request] : " + `SELECT con.mobile_no, con.bearer_token, con.phone_number_id, con.rcs_business_acc_id FROM user_management usr
        LEFT JOIN rcs_config con ON con.user_id = usr.user_id
        WHERE concat(con.country_code, con.mobile_no) = '${mobile_number}' AND con.is_qr_code = 'N'`)
    const get_user_number = await db.query(`SELECT con.mobile_no, con.bearer_token, con.phone_number_id, con.rcs_business_acc_id FROM user_management usr
        LEFT JOIN rcs_config con ON con.user_id = usr.user_id
        WHERE concat(con.country_code, con.mobile_no) = '${mobile_number}' AND con.is_qr_code = 'N'`);
    ...
    if (get_user_number.length == 0) {
      ...
      var config = {
        method: 'get',
        url: `${api_url}${get_user_number[0].rcs_business_acc_id}/message_templates?name=${template_name}`,
        headers: {
